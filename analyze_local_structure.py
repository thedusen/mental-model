    ...
    try:
        ...
        with driver.session() as session:
            ...
            try:
                ...
                """)
                for record in result:
                    logger.info(f"  {record['relationshipType']}: {record['count']} relationships")
            except:
                # Fallback without APOC
            ...
            """)
            for record in result:
                logger.info(f"  ({record['from_label']})-[:{record['rel_type']}]->({record['to_label']})")
            ...
            """)
            for record in result:
                logger.info(f"  {record['rel_type']}: {record['count']} Entity->Entity relationships")
            ...
            """)
            for record in result:
                logger.info(f"  {record['id']}: {record['labels']} -> {record['properties']}")
