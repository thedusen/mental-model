    ...
    try:
        ...
        with driver.session() as session:
            ...
            sample_entities = session.run(
                "MATCH (n:Entity) RETURN n.id as id LIMIT 5"
            ).values()
...
def main():
    ...
    if local_result["success"] and aura_result["success"]:
        ...
        if local_result["embedded_count"] > aura_result["embedded_count"]:
            logger.warning(
                f"⚠️  Local database has {local_result['embedded_count'] - aura_result['embedded_count']} more entities with embeddings than AuraDB"
            )
            ...
            logger.info(
                "✅ Both databases have the same number of entities with embeddings"
            )
        else:
        ...
        logger.info(
            f"Sample entity overlap: {len(overlap)}/{len(local_sample)} entities"
        )
