...
class HealthChecker:
    ...
    async def test_supabase_connection(self):
        ...
        try:
            ...
            response = self.supabase.client.from_("questionnaire_questions").select("count").execute()
    ...
    async def test_questionnaire_schema(self):
        ...
        try:
            ...
            for table in tables_to_check:
                try:
                    response = self.supabase.client.from_(table).select("*").limit(1).execute()
    ...
    async def setup_test_questions(self):
        ...
        try:
            ...
            response = self.supabase.client.from_("questionnaire_questions").select("*").execute()
    ...
    async def test_question_retrieval(self):
        ...
        try:
            response = self.supabase.client.from_("questionnaire_questions").select("*").order("question_number").execute()
    ...
    async def test_user_creation(self):
        ...
        try:
            ...
            response = self.supabase.client.from_("user_profiles").insert(user_data).execute()
    ...
    async def test_questionnaire_flow(self):
        ...
        try:
            # Get first question
            questions_response = self.supabase.client.from_("questionnaire_questions").select("*").order("question_number").limit(3).execute()
            ...
            for i, (question, answer) in enumerate(zip(questions[:3], test_answers)):
                try:
                    ...
                    response = self.supabase.client.from_("user_questionnaire_responses").insert(response_data).execute()
            ...
            try:
                progress_response = self.supabase.client.from_("user_questionnaire_progress").select("*").eq("user_id", TEST_USER_ID).execute()
                
                if progress_response.data:
                    progress = progress_response.data[0]
                    print(f"✅ Progress tracked: {progress['status']}, current question: {progress['current_question']}")
                else:
    ...
    async def test_zep_integration(self):
        ...
        try:
            ...
            # Try to sync to Zep
            await self.zep.add_or_update_business_context(TEST_USER_ID, test_entity_data)
            ...
            if business_context:
                print(f"✅ Retrieved business context from Zep: {business_context[:100]}...")
                self.results['zep_integration'] = True
            else:
                print("⚠️  No business context retrieved from Zep yet (may need time to process)")
    ...
    async def test_context_loading(self):
        ...
        try:
            ...
            if self.zep.enabled:
                ...
                if business_profile:
                    print(f"✅ Business profile loaded for context: {list(business_profile.keys())}")
                ...
                responses = self.supabase.client.from_("user_questionnaire_responses").select("*, questionnaire_questions(question_text, question_category)").eq("user_id", TEST_USER_ID).execute()
                
                if responses.data:
                    print(f"✅ Retrieved {len(responses.data)} questionnaire responses for context")
                    self.results['context_loading'] = True
                    
                    # Show sample context
                    for resp in responses.data[:2]:
                        print(f"  - {resp['questionnaire_questions']['question_text'][:30]}...: {resp['response_text'][:30]}...")
                else:
    ...
    async def cleanup_test_data(self):
        ...
        try:
            ...
            self.supabase.client.from_("user_questionnaire_responses").delete().eq("user_id", TEST_USER_ID).execute()
            
            # Delete test progress
            self.supabase.client.from_("user_questionnaire_progress").delete().eq("user_id", TEST_USER_ID).execute()
            
            # Delete test user
            self.supabase.client.from_("user_profiles").delete().eq("id", TEST_USER_ID).execute()
    ...
    def print_summary(self):
        ...
        for test_name, result in self.results.items():
            total += 1
            status_emoji = "✅" if result is True else ("⚠️" if result == 'partial' or result == 'disabled' else "❌")
            status_text = str(result) if result != True and result != False else ("PASS" if result else "FAIL")
