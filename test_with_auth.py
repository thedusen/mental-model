...
class AuthenticatedHealthChecker:
    ...
    async def test_authenticated_questionnaire(self):
        ...
        try:
            ...
            questions_response = self.admin_client.table("questionnaire_questions").select("*").order("question_number").limit(3).execute()
            ...
            for i, (question, answer) in enumerate(zip(questions, test_answers)):
                try:
                    ...
                    if response.data:
                        saved_count += 1
                        print(f"✅ Saved Q{question['question_number']}: {answer[:40]}...")
            ...
            print(f"📊 Questionnaire test result: {saved_count}/{len(questions)} answers saved")
            ...
            if progress_response.data:
                ...
                print(f"✅ Progress tracking works: Status={progress['status']}, Current Q={progress['current_question']}")
            else:
    ...
    async def test_zep_with_auth(self):
        ...
        try:
            ...
            zep_user = zep_service.manager.ensure_user_exists(self.test_user.id, user_metadata)
            ...
            business_context = await zep_service.get_business_profile_context(self.test_user.id)
    ...
    async def simulate_chat_integration(self):
        ...
        try:
            ...
            supabase_context = ""
            if responses.data:
                relevant_responses = [r for r in responses.data if 'challenge' in r['questionnaire_questions']['question_text'].lower()]
                
                if relevant_responses:
                    for resp in relevant_responses:
                        supabase_context += f"User's Challenge: {resp['response_text']}\n"
