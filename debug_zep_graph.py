...
async def debug_zep_graph():
    ...
    try:
        ...
        result = zep_service.client.graph.add(
            user_id=test_user_id, data=json.dumps(test_data), type="json"
        )
        ...
        for i, node in enumerate(nodes):
            ...
            print(
                f"      All attributes: {[attr for attr in dir(node) if not attr.startswith('_')]}"
            )
        ...
        test_content = (
            "Business Profile Context: Q1 (test): Test question\nAnswer: Test answer"
        )
        ...
        questionnaire_entities = await zep_service.get_questionnaire_entities(
            test_user_id
        )
        ...
        if questionnaire_entities:
            ...
            for entity in questionnaire_entities:
                print(
                    f"   Entity: {entity.get('entity_id')} - {entity.get('answer', 'No answer')}"
                )
        else:
