...
class EmbeddingManager:
    ...
    def should_generate_embedding(self, node_data, existing_embeddings=None):
        ...
        description = node_data['properties'].get('description') or node_data['properties'].get('content', '')
    ...
    def generate_embedding_with_retry(self, text, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(f"⏱️ Rate limit hit. Waiting {wait_time} seconds (attempt {attempt + 1})...")
                time.sleep(wait_time)
                
            except Exception as e:
                logger.error(f"❌ Embedding generation failed (attempt {attempt + 1}): {str(e)}")
    ...
    def process_nodes_for_embeddings(self, nodes, existing_embeddings=None):
        ...
        for i, node in enumerate(nodes):
            ...
            should_generate, reason = self.should_generate_embedding(node, existing_embeddings)
            ...
            if should_generate:
                ...
                if embedding:
                    ...
                    node['properties']['embedding_generated_at'] = datetime.now().isoformat()
            ...
            with auradb_driver.session() as session:
                ...
                for record in result:
                    ...
                    if entity_id:
                        ...
                        current_hash = self.get_text_hash(description) if description else None
                ...
                logger.info(f"✅ Extracted {len(existing_embeddings)} existing embeddings from AuraDB")
    ...
    def optimize_embedding_strategy(self, local_nodes, auradb_driver=None):
        ...
        existing_embeddings = {}
        if auradb_driver:
            existing_embeddings = self.extract_existing_embeddings_from_auradb(auradb_driver)
    ...
    def validate_embeddings(self, nodes):
        ...
        for node in nodes:
            ...
            if isinstance(embedding, list) and len(embedding) == 1024:
                ...
                logger.warning(f"⚠️ Invalid embedding for {node['properties'].get('id')}")
