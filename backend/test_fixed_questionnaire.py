...
def test_fail_fast_behavior():
    ...
    try:
        ...
        try:
            ...
            print(
                f"   ✅ Good: Invalid API key correctly failed with: {str(auth_error)[:100]}..."
            )

    except Exception as e:
        print(
            f"   ✅ Good: Service correctly failed with invalid config: {str(e)[:100]}..."
        )
    ...
    try:
        ...
        users = client.user.list_ordered(page_size=1)
        print(
            f"   ✅ Valid API key works: Found {len(users.users) if users.users else 0} users"
        )
