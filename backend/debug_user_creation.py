...
def test_questionnaire_user_creation():
    ...
    try:
        ...
        try:
            ...
            user_ids = [user.user_id for user in users_response.users] if users_response.users else []
        ...
        try:
            ...
            if hasattr(graph_data, 'nodes') and graph_data.nodes:
                ...
                for node in graph_data.nodes[:3]:
                    print(f"      - {getattr(node, 'name', 'Unknown')} ({getattr(node, 'node_type', 'Unknown type')})")
            else:
