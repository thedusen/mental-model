...
class SyncValidator:
    ...
    def get_database_stats(self, uri, username, password, db_name):
        ...
        try:
            ...
            with driver.session() as session:
                ...
                entities_with_embeddings = session.run("MATCH (n:Entity) WHERE n.embedding IS NOT NULL RETURN count(n) as count").single()["count"]
    ...
    def validate_data_integrity(self, uri, username, password, db_name):
        ...
        try:
            ...
            with driver.session() as session:
                ...
                result = session.run("MATCH (n:Entity) WHERE n.id IS NULL RETURN count(n) as count").single()
                entities_without_ids = result["count"]
                if entities_without_ids > 0:
                    issues.append(f"❌ {entities_without_ids} entities without ID property")
                
                # Check for entities without embeddings
                result = session.run("MATCH (n:Entity) WHERE n.embedding IS NULL RETURN count(n) as count").single()
                entities_without_embeddings = result["count"]
                if entities_without_embeddings > 0:
                    issues.append(f"⚠️ {entities_without_embeddings} entities without embeddings")
                
                # Check for orphaned entities (no theme relationship)
                result = session.run("MATCH (n:Entity) WHERE NOT (n)-[:BELONGS_TO]->(:Theme) RETURN count(n) as count").single()
                orphaned_entities = result["count"]
                if orphaned_entities > 0:
                    issues.append(f"⚠️ {orphaned_entities} entities not connected to themes")
                
                # Check for themes without entities
                result = session.run("MATCH (t:Theme) WHERE NOT (:Entity)-[:BELONGS_TO]->(t) RETURN count(t) as count").single()
                ...
                if invalid_embeddings > 0:
                    issues.append(f"❌ {invalid_embeddings} entities with invalid embedding dimensions")
    ...
    def compare_databases(self, pre_sync_aura_stats=None):
        ...
        comparison['differences'] = {
            ...
            'rel_count_diff': local_stats['total_relationships'] - aura_stats['total_relationships'],
            'entity_count_diff': local_stats['entity_count'] - aura_stats['entity_count'],
            'embedding_count_diff': local_stats['entities_with_embeddings'] - aura_stats['entities_with_embeddings'],
        ...
        if pre_sync_aura_stats:
            comparison['sync_effectiveness'] = {
                'nodes_synced': aura_stats['total_nodes'] - pre_sync_aura_stats['total_nodes'],
                'relationships_synced': aura_stats['total_relationships'] - pre_sync_aura_stats['total_relationships'],
                'entities_synced': aura_stats['entity_count'] - pre_sync_aura_stats['entity_count'],
                'embeddings_synced': aura_stats['entities_with_embeddings'] - pre_sync_aura_stats['entities_with_embeddings']
            }
        
        # Log comparison results
        logger.info("📊 Database comparison results:")
        logger.info(f"   LOCAL:  {local_stats['total_nodes']} nodes, {local_stats['total_relationships']} relationships")
        logger.info(f"   AuraDB: {aura_stats['total_nodes']} nodes, {aura_stats['total_relationships']} relationships")
        logger.info(f"   Differences: {comparison['differences']['node_count_diff']} nodes, {comparison['differences']['rel_count_diff']} relationships")
    ...
    def validate_specific_entities(self, sample_entity_ids):
        ...
        validation_results = {'matches': 0, 'mismatches': 0, 'missing_in_aura': 0, 'details': []}
        
        try:
            # Connect to both databases
            local_driver = GraphDatabase.driver(self.local_uri, auth=(self.local_username, self.local_password))
            aura_driver = GraphDatabase.driver(self.aura_uri, auth=(self.aura_username, self.aura_password))
            ...
            for entity_id in sample_entity_ids:
                ...
                if not aura_result:
                    ...
                    validation_results['details'].append(f"❌ {entity_id}: Missing in AuraDB")
                    ...
                    if local_labels == aura_labels:
                        ...
                        validation_results['details'].append(f"✅ {entity_id}: Labels match")
                    else:
                        validation_results['mismatches'] += 1
                        validation_results['details'].append(f"⚠️ {entity_id}: Label mismatch - Local: {local_labels}, AuraDB: {aura_labels}")
        ...
        logger.info(f"📊 Entity validation: {validation_results['matches']} matches, {validation_results['mismatches']} mismatches, {validation_results['missing_in_aura']} missing")
        
        return validation_results
    
    def perform_complete_validation(self, pre_sync_aura_stats=None, sample_entities=None):
        ...
        # Compare databases
        validation_report['database_comparison'] = self.compare_databases(pre_sync_aura_stats)
        ...
        if sample_entities:
            validation_report['entity_validation'] = self.validate_specific_entities(sample_entities)
        
        # Determine overall status
        perfect_sync = validation_report['database_comparison']['perfect_sync'] if validation_report['database_comparison'] else False
        no_integrity_issues = (len(validation_report['local_integrity']) == 0 and 
                              len(validation_report['aura_integrity']) == 0)
        ...
        if perfect_sync and no_integrity_issues:
            ...
            logger.warning("⚠️ VALIDATION PASSED WITH WARNINGS: Sync completed but integrity issues found")
        else:
