...
class AuraDBAnalyzer:
    ...
    def analyze_nodes(self):
        ...
        with self.driver.session() as session:
            ...
            return {"nodes": nodes, "entity_lookup": node_by_entity_id}
    ...
    def analyze_relationships(self):
        ...
        with self.driver.session() as session:
            ...
            rel_by_entities = (
                {}
            )  # Map (from_entity_id, to_entity_id, rel_type) to relationship
    ...
    def get_database_stats(self):
        ...
        with self.driver.session() as session:
            ...
            logger.info(
                f"✅ AuraDB stats: {node_count} nodes, {rel_count} relationships"
            )
            return stats
        ...
        try:
            ...
            analysis_data = {
                ...
                "lookups": {
                    ...
                    "relationship_by_entities": rel_analysis[
                        "entity_relationship_lookup"
                    ],
                },
            }
            ...
            logger.info(
                f"📊 Analysis summary: {len(node_analysis['nodes'])} nodes, {len(rel_analysis['relationships'])} relationships"
            )
    ...
    def compare_with_local_export(self, local_export_data):
        ...
        comparison = {
            ...
            "statistics_comparison": {
                ...
                "auradb": auradb_stats,
                "differences": {
                    "node_count_diff": local_stats["total_nodes"]
                    - auradb_stats["total_nodes"],
                    "rel_count_diff": local_stats["total_relationships"]
                    - auradb_stats["total_relationships"],
                },
            },
            "missing_in_auradb": {"nodes": [], "relationships": []},
            "extra_in_auradb": {"nodes": [], "relationships": []},
        }
        ...
        logger.info(
            f"   - Node count difference: {comparison['statistics_comparison']['differences']['node_count_diff']}"
        )
        logger.info(
            f"   - Relationship count difference: {comparison['statistics_comparison']['differences']['rel_count_diff']}"
        )
...
def main():
    ...
    if result:
        ...
        print(
            f"   - {result['statistics']['total_relationships']} relationships analyzed"
        )
        print(f"   - Label counts: {result['statistics']['label_counts']}")
        print(
            f"   - Relationship type counts: {result['statistics']['relationship_type_counts']}"
        )
    else:
