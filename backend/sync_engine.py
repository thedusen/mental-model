...
class SyncEngine:
    ...
    def generate_embedding(self, text):
        ...
        if not self.cohere_client:
            logger.warning(
                "⚠️ No Cohere client available - skipping embedding generation"
            )
            return None
    ...
    def sync_nodes_batch(self, local_nodes, batch_size=50):
        ...
        logger.info(
            f"📦 Syncing {len(local_nodes)} nodes in batches of {batch_size}..."
        )
        ...
        with self.driver.session() as session:
            for i in range(0, len(local_nodes), batch_size):
                ...
                logger.info(
                    f"  Processing batch {i//batch_size + 1}/{(len(local_nodes) + batch_size - 1)//batch_size}"
                )
                ...
                for node in batch:
                    ...
                    needs_embedding = (
                        "embedding" not in node["properties"] and description
                    )
        ...
        logger.info(
            f"✅ Nodes sync complete: {nodes_created} created, {nodes_updated} updated"
        )
    ...
    def sync_relationships_batch(self, local_relationships, batch_size=100):
        ...
        logger.info(
            f"🔗 Syncing {len(local_relationships)} relationships in batches of {batch_size}..."
        )
        ...
        with self.driver.session() as session:
            for i in range(0, len(local_relationships), batch_size):
                ...
                logger.info(
                    f"  Processing relationship batch {i//batch_size + 1}/{(len(local_relationships) + batch_size - 1)//batch_size}"
                )
                ...
                for rel in batch:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Relationship sync issue (may be normal): {str(e)}"
                        )

        logger.info(
            f"✅ Relationships sync complete: {relationships_created} processed"
        )
    ...
    def sync_entity_relationships_by_id(self, local_export_data):
        ...
        logger.info(
            f"✅ Entity relationships sync complete: {relationships_created} created"
        )
        return relationships_created
    ...
    def perform_complete_sync(self, local_export_data, dry_run=False):
        ...
        try:
            ...
            # Sync entity nodes
            nodes_created, nodes_updated = self.sync_nodes_batch(
                local_export_data["nodes"]
            )

            # Sync relationships
            relationships_created = self.sync_entity_relationships_by_id(
                local_export_data
            )

            logger.info("✅ Complete sync finished!")
            logger.info(
                f"   📊 Summary: {nodes_created} nodes created, {nodes_updated} nodes updated"
            )
    ...
    def analyze_sync_changes(self, local_export_data):
        ...
        logger.info(
            f"   - Relationships to create: {analysis['relationships_to_create']}"
        )
