...
def main():
    ...
    try:
        ...
        user_methods = [
            method for method in dir(client.user) if not method.startswith("_")
        ]
        ...
        try:
            ...
            print(
                f"   👤 Created user: {user.user_id if hasattr(user, 'user_id') else 'Unknown'}"
            )
            ...
            try:
                if hasattr(client.user, method_name):
                    ...
                    try:
                        result = method()
                        print(
                            f"   ✅ user.{method_name}() works! Result type: {type(result)}"
                        )
                        if hasattr(result, "users"):
                            print(
                                f"   📊 Users count: {len(result.users) if result.users else 0}"
                            )
                    except Exception as e:
                        ...
                        try:
                            ...
                            print(
                                f"   ✅ user.{method_name}(limit=5) works! Result type: {type(result)}"
                            )
                            if hasattr(result, "users"):
                                print(
                                    f"   📊 Users count: {len(result.users) if result.users else 0}"
                            ...
                            try:
                                ...
                                print(
                                    f"   ✅ user.{method_name}(page_size=5) works! Result type: {type(result)}"
                                )
                                if hasattr(result, "users"):
                                    print(
                                        f"   📊 Users count: {len(result.users) if result.users else 0}"
        ...
        if created_user_id:
            ...
            try:
                ...
                print(
                    f"   👤 Retrieved: {retrieved_user.user_id if hasattr(retrieved_user, 'user_id') else 'Unknown'}"
                )
            except Exception as e:
