...
def categorize_uncategorized_nodes():
    ...
    with get_db_session() as session:
        ...
        for node in uncategorized_nodes:
            ...
            for category, keywords in categorization_rules.items():
                ...
                for keyword in keywords:
                    ...
                    score += len(
                        re.findall(
                            r"\b" + re.escape(keyword.lower()) + r"\b", text_to_analyze
                        )
                    )
            ...
            if best_category[1] > 0:  # Only assign if we found matching keywords
                ...
                categorization_counts[category] = (
                    categorization_counts.get(category, 0) + 1
                )
                updated_count += 1

                print(
                    f"Categorized '{node_id}' as {category} (score: {best_category[1]})"
                )
        ...
        result = session.run(
            "MATCH (e:Entity) WHERE e.category IS NULL RETURN count(*) as count"
        )
