...
class SyncOrchestrator:
    ...
    def export_local_database(self):
        ...
        self.local_export_data = self.local_exporter.export_complete_database(
            export_file
        )
        ...
        logger.info(
            f"✅ Local database exported: {len(self.local_export_data['nodes'])} nodes, {len(self.local_export_data['relationships'])} relationships"
        )
        return True
    ...
    def analyze_auradb_current_state(self):
        ...
        logger.info(
            f"✅ AuraDB analyzed: {self.pre_sync_aura_stats['total_nodes']} nodes, {self.pre_sync_aura_stats['total_relationships']} relationships"
        )
        ...
        rel_diff = (
            ...
            - self.pre_sync_aura_stats["total_relationships"]
        )

        logger.info(
            f"📊 Sync needed: {node_diff} nodes, {rel_diff} relationships to sync"
        )
    ...
    def optimize_embeddings(self):
        ...
        try:
            ...
            embedding_stats = self.embedding_manager.optimize_embedding_strategy(
                self.local_export_data["nodes"], auradb_driver
            )

            logger.info(
                f"✅ Embedding optimization complete: {embedding_stats['embeddings_generated']} new, {embedding_stats['embeddings_reused']} reused"
            )
    ...
    def perform_sync(self):
        ...
        if self.dry_run:
            ...
            self.sync_results = self.sync_engine.perform_complete_sync(
                self.local_export_data, dry_run=True
            )
        else:
            logger.info("🚀 STEP 4: Performing LIVE SYNC to AuraDB...")
            self.sync_results = self.sync_engine.perform_complete_sync(
                self.local_export_data, dry_run=False
            )
    ...
    def generate_sync_report(self):
        ...
        report = {
            ...
            "local_export_summary": {
                "total_nodes": (
                    len(self.local_export_data["nodes"])
                    if self.local_export_data
                    else 0
                ),
                "total_relationships": (
                    len(self.local_export_data["relationships"])
                    if self.local_export_data
                    else 0
                ),
                "statistics": (
                    self.local_export_data["statistics"]
                    if self.local_export_data
                    else None
                ),
            },
            ...
            "sync_status": (
                "SUCCESS"
                if self.sync_results and self.sync_results.get("success")
                else "FAILED"
            ),
        }
        ...
        if self.dry_run:
            ...
            if self.sync_results:
                logger.info(
                    f"   Would create: {self.sync_results.get('entities_to_create', 0)} entities"
                )
                logger.info(
                    f"   Would update: {self.sync_results.get('entities_to_update', 0)} entities"
                )
                logger.info(
                    f"   Would create: {self.sync_results.get('relationships_to_create', 0)} relationships"
                )
        else:
            logger.info("   Mode: LIVE SYNC")
            if self.sync_results:
                logger.info(
                    f"   Nodes created: {self.sync_results.get('nodes_created', 0)}"
                )
                logger.info(
                    f"   Nodes updated: {self.sync_results.get('nodes_updated', 0)}"
                )
                logger.info(
                    f"   Relationships created: {self.sync_results.get('relationships_created', 0)}"
                )
