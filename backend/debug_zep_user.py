            ...
            if hasattr(entities, "entities") and entities.entities:
                ...
                for entity in entities.entities:
                    if (
                        ...
                        and "business_profile" in str(entity.entity_type).lower()
                    ):
                ...
                if business_profile_entities:
                    ...
                    for entity in sorted(
                        business_profile_entities,
                        key=lambda x: getattr(x, "entity_id", ""),
                    ):
                        ...
                        if hasattr(entity, "data"):
                            ...
                            question_text = (
                                data.get("question", "No question text")[:50] + "..."
                            )
        ...
        try:
            ...
            if hasattr(memory, "messages") and memory.messages:
                ...
                questionnaire_messages = [
                    ...
                    if "questionnaire" in str(m).lower() or "business" in str(m).lower()
                ]
                print(
                    f"   Questionnaire-related messages: {len(questionnaire_messages)}"
                )
            else:
        ...
        if response.data:
            ...
            for user_id, responses in users.items():
                ...
                for resp in sorted(responses, key=lambda x: x.get("question_id", 0)):
                    q_num = resp.get("questionnaire_questions", {}).get(
                        "question_number", "Unknown"
                    )
                    q_text = (
                        resp.get("questionnaire_questions", {}).get(
