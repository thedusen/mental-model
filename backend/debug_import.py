...
def test_sample_import():
    ...
    with get_db_session() as session:
        ...
        logger.info(f"Current database state: {node_count} total nodes, {entity_count} entities")
                ...
                if isinstance(nodes, list):
                    ...
                    for node_idx, node in enumerate(nodes[:3]):  # Only first 3 nodes
                        ...
                        if not all([entity_id, description, theme_name]):
                            logger.warning(f"Skipping node due to missing data: {entity_id}")
                            continue
                        ...
                        logger.debug(f"Creating entity with labels: Entity, {entity_label}")
                        ...
                        # Create relationship
                        logger.debug(f"Creating relationship: {entity_id} -> {theme_name}")
                        rel_query = """
                        ...
                        nodes_processed += 1
                        logger.info(f"✅ Successfully imported '{entity_id}' as {entity_label}")
                        ...
                        if verify_result:
                            logger.debug(f"Verification - Labels: {verify_result['labels']}, Has embedding: {verify_result['has_embedding']}")
                        else:
                            logger.error(f"❌ Node verification failed for: {entity_id}")
    
    logger.info(f"🏁 Debug import completed: {nodes_processed} processed, {nodes_skipped} skipped")
