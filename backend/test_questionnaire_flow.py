...
os.environ["ZEP_API_KEY"] = (
    "z_1dWlkIjoiZGIzYTMxYzQtNjVlMi00NDM1LTlmMjgtZWY3ZTNkZTE5YzM2In0.frpMBktBNV6Yyo080wnj09heynO2Mg-pPACeJj_ge8lZ5GCH1I1GdW6xtMHhL1VzBn3y6WoyzpcJCmVKVcI9aA"
)
            ...
            try:
                ...
                if hasattr(graph_data, "nodes") and graph_data.nodes:
                    ...
                    for i, node in enumerate(graph_data.nodes[:3], 1):
                        print(
                            f"      {i}. {getattr(node, 'name', 'Unknown')} - {getattr(node, 'node_type', 'Unknown type')}"
                        )
                else:
