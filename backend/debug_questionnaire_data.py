    ...
    try:
        ...
        if not response.data:
            ...
            for resp in response.data:
                ...
                print(f"     📝 Q{question_info.get('question_number', 'Unknown')}: {question_info.get('question_text', 'Unknown question')[:60]}...")
            ...
            for user_id in users_found:
                try:
                    ...
                    knowledge_graph = zep_service.manager.get_user_knowledge_graph(user_id)
                    ...
                    if knowledge_graph and 'nodes' in knowledge_graph:
                        ...
                        for i, node in enumerate(nodes[:3]):
                            data = node.get('data', {})
                            print(f"       {i+1}. Type: {data.get('entity_type', 'unknown')}")
                            print(f"          ID: {data.get('entity_id', 'unknown')}")
                            print(f"          Data keys: {list(data.keys())}")
                        
                        business_nodes = [
                            node for node in nodes 
                            if node.get('data', {}).get('entity_type') == "business_profile_question"
                        ]
                        ...
                        for node in business_nodes[:5]:  # Show first 5
                            data = node.get('data', {})
                            print(f"       • {data.get('entity_id', 'Unknown')}: {data.get('question', 'Unknown')[:40]}...")
                            print(f"         Answer: {data.get('answer', 'No answer')[:60]}...")
                        
                        if len(business_nodes) > 5:
                            print(f"       ... and {len(business_nodes) - 5} more")
                    else:
                        print(f"  👤 User: {user_id[:8]}... - No Zep knowledge graph found or empty")
