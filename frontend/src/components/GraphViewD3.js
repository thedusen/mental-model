...
function GraphViewD3({ onNodeSelect, onCanvasClick, chatContextNode, filters }) {
  console.log('GraphViewD3 component rendering...');
  ...
  console.log('Rendering GraphViewD3 with data:', memoizedNodes.length, 'nodes');
  ...
  return (
    <div 
      ...
      tabIndex="0"
      role="application"
      aria-label="Knowledge graph visualization. Use + and - keys to zoom, click nodes to select them."
    >
      ...
      <div style={{
        ...
        >
          ...
          <button 
            ...
            aria-label={isLegendMinimized ? 'Expand legend' : 'Collapse legend'}
          >
            ▼
          </button>
        </div>
          ...
          <div style={{
            ...
            {chatContextNode && (
              <div style={{
                ...
                🗨️ Chat Context: {chatContextNode.properties?.name || chatContextNode.name || 'Active'}
              </div>
            )}
          </div>
        )}
      ...
      {/* 
        Conditionally render the NVL component only when graph data is available.
      */}
      {memoizedNodes.length > 0 && (
        <NVL
          ...
          legend={false} // This will hide the default NVL legend
