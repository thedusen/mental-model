...
class CompleteFlowTester:
    ...
    async def answer_key_questions(self):
        ...
        # Get all questions
        questions_response = self.admin_client.table("questionnaire_questions").select("*").order("question_number").execute()
        ...
        for question in questions:
            ...
            if question_num in business_answers:
                try:
                    ...
                    response = self.admin_client.table("user_questionnaire_responses").insert(response_data).execute()
        ...
        progress_response = self.admin_client.table("user_questionnaire_progress").select("*").eq("user_id", self.test_user.id).execute()
        
        if progress_response.data:
            progress = progress_response.data[0]
            print(f"✅ Progress: Status={progress['status']}, Questions completed: {saved_count}/11")
    ...
    async def sync_answer_to_zep(self, question: Dict, answer: str):
        ...
        try:
            ...
            await zep_service.add_or_update_business_context(self.test_user.id, entity_data)
    ...
    async def test_business_context_queries(self):
        ...
        for query, expected_category in test_queries:
            ...
            responses = self.admin_client.table("user_questionnaire_responses").select("*, questionnaire_questions(question_text, question_category)").eq("user_id", self.test_user.id).execute()
            ...
            if responses.data:
                ...
                for resp in responses.data:
                    ...
                    question_text = resp['questionnaire_questions']['question_text'].lower()
                    
                    # Match by category or keywords
                    if (question_cat == expected_category or 
                        any(word in question_text for word in query.lower().split() if len(word) > 3)):
                        relevant_responses.append(resp)
                
                if relevant_responses:
                    print(f"✅ Found {len(relevant_responses)} relevant responses from Supabase:")
            ...
            if zep_service.enabled:
                try:
                    business_profile = zep_service.manager.get_business_profile(self.test_user.id)
                    
                    if business_profile:
                        print(f"✅ Zep business profile has {len(business_profile)} elements:")
    ...
    async def test_real_chat_flow(self):
        ...
        test_query = "What are my biggest business challenges and how can I address them?"
        ...
        try:
                    ...
                    if business_profile:
                        ...
                        for key, value in business_profile.items():
                            if any(word in key.lower() or word in value.lower() 
                                   for word in ['challenge', 'problem', 'difficult', 'issue']):
                                relevant_elements.append(f"- {key.replace('_', ' ').title()}: {value}")
                        
                        if relevant_elements:
                            business_context = "Business Profile Context:\n" + "\n".join(relevant_elements)
                            user_context_parts.append(business_context)
                            print(f"✅ Zep business context ({len(business_context)} chars)")
            ...
            if not user_context_parts:
                responses = self.admin_client.table("user_questionnaire_responses").select("*, questionnaire_questions(question_text, question_category)").eq("user_id", user_id).execute()
                
                if responses.data:
                    # Find challenge-related responses
                    challenge_responses = [
                        r for r in responses.data 
                        if 'challenge' in r['questionnaire_questions']['question_text'].lower() or
                           r['questionnaire_questions']['question_category'] == 'challenges'
                    ]
                    
                    if challenge_responses:
                        context_items = []
                        for resp in challenge_responses:
                            context_items.append(f"- Business Challenge: {resp['response_text']}")
                        
                        business_context = "Business Profile Context:\n" + "\n".join(context_items)
                        user_context_parts.append(business_context)
                        print(f"✅ Supabase business context ({len(business_context)} chars)")
            ...
            combined_user_context = "\n\n".join(user_context_parts) if user_context_parts else "No user context available"
            
            print(f"\n✅ Final combined user context ({len(combined_user_context)} chars):")
            print("─" * 60)
            print(combined_user_context[:400] + ("..." if len(combined_user_context) > 400 else ""))
            print("─" * 60)
    ...
    async def cleanup_test_user(self):
        ...
        try:
            ...
            self.admin_client.table("user_questionnaire_responses").delete().eq("user_id", user_id).execute()
            self.admin_client.table("user_questionnaire_progress").delete().eq("user_id", user_id).execute()
            self.admin_client.table("user_profiles").delete().eq("id", user_id).execute()
