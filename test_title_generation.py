...
def test_title_generation_via_api():
    ...
    try:
        ...
        session_response = requests.post(
            f"{api_url}/api/chat/sessions", json={"user_id": test_user_id}
        )
        ...
        if user_message_response.status_code != 200:
            print(
                f"❌ Failed to add user message: {user_message_response.status_code} - {user_message_response.text}"
            )
            return
        ...
        print(
            "3. Adding assistant message via API (should trigger title generation)..."
        )
        assistant_message_response = requests.post(
            f"{api_url}/api/chat/messages",
            json={
                "session_id": session_id,
                "role": "assistant",
        ...
        if assistant_message_response.status_code != 200:
            print(
                f"❌ Failed to add assistant message: {assistant_message_response.status_code} - {assistant_message_response.text}"
            )
            return
        ...
        if sessions_response.status_code != 200:
            print(
                f"❌ Failed to get sessions: {sessions_response.status_code} - {sessions_response.text}"
            )
            return
        ...
        if (
            target_session
            and target_session.get("title")
            and target_session["title"] != "Untitled conversation"
        ):
